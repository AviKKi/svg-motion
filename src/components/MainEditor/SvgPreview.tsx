...
export function SvgPreview({}: SvgPreviewProps) {
  ...
  const { svgContent, svgUri, svgName, animations, isPlaying, currentTime, setCurrentTime, setSvgContent, setSvgUri, setSvgName, clearSvg } = useAnimationStore()
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      switch (type) {
        case 'iframe-ready':
          ...
          break
        case 'time-update':
          Math.abs(currentTime-frameCurrentTime) > 10 && setCurrentTime(frameCurrentTime)
          break
  ...
  useEffect(() => {
    const loadSvgContent = async () => {
      ...
      try {
        if (uploadedFile) {
          ...
          const name = selectedSampleSvg.split('/').pop()?.replace('.svg', '') || ''
          setSvgName(name)
        }
      } catch (error) {
    ...
    loadSvgContent()
  }, [uploadedFile, selectedSampleSvg, setSvgContent, setSvgUri, setSvgName, clearSvg])
